...
def put_final_result(filename):
    ...
    for i in range(len(df)):
        df.loc[i,'Overall Match'] = (float(df.loc[i,'UID Match Score']) + float(df.loc[i,'Final Address Match Score']) + float(df.loc[i,'Final Address Match Score'])) / 3
        ...
        if float(df.loc[i,'UID Match Score']) < 100 or float(df.loc[i,'Final Address Match Score']) < 80 or float(df.loc[i,'Final Address Match Score']) < 85:
            df.loc[i,'Final Remarks'] = "Couldn't Verify Your aadhar card ."
